...
import type { PassengerFormProps ,Location, Passenger, Route } from '../types';
import AutocompleteSearch from './AutocompleteSearch';
import { api } from '../services/api';
import { validateRoute } from '../services/validroute.ts';

import axios from 'axios';
// import {isLocationNearRoute} from './isLocationNear';
...
//  // Call the validateRoute API to check if the locations are valid
//  try {
//    // Convert the resolvedPickup to match the expected 'lon' property
//    const resolvedPickupWithLon = {
//     ...resolvedPickup,
//     lon: resolvedPickup.lng, // Convert lng to lon
// };

// // Similarly for drop location
// const resolvedDropWithLon = {
//     ...resolvedDrop,
//     lon: resolvedDrop.lng, // Convert lng to lon
// };
// // Convert Route to match the expected OwnerRoute format
// const resolvedMainRoute = {
//   ...mainRoute,
//   startLocation: {
//     ...mainRoute.startLocation,
//     lon: mainRoute.startLocation.lng,  // Convert lng to lon for startLocation
//   },
//   endLocation: {
//     ...mainRoute.endLocation,
//     lon: mainRoute.endLocation.lng,  // Convert lng to lon for endLocation
//   },
// };
//   const validationResponse = await validateRoute(resolvedPickupWithLon, resolvedDropWithLon, resolvedMainRoute);

//   if (validationResponse.isValid) {
//     setValidationMessage({
//       type: 'success',
//       message: 'Route is valid! Calculating optimal cost...'
//     });
//       // Prepare the new passenger data
//       // await calculateOptimizedCost(resolvedPickup, resolvedDrop);

//       // Optionally, you can calculate optimized cost here
//       await calculateOptimizedCost(resolvedPickup, resolvedDrop);
//         const newPassenger = {
//       name,
...
//     };
//     await axios.post(`https://rideok-v6.onrender.com/api/rides/${rideId}/passenger`, newPassenger);
...
//     type: 'error',
//     message: 'This route is not eligible for carpooling. Please ensure your pickup and drop points are along the driver\'s route and in the same direction.'
//   });
//   }
// } catch (error) {
//   console.error(error);
...
const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Prepare locations with correct property names
      const resolvedPickupWithLon = {
        ...resolvedPickup,
        lon: resolvedPickup.lng,
      };

      const resolvedDropWithLon = {
        ...resolvedDrop,
        lon: resolvedDrop.lng,
      };
      ...
      const resolvedMainRoute = {
        ...
        startLocation: {
          ...mainRoute.startLocation,
          lon: mainRoute.startLocation.lng,
        },
        endLocation: {
          ...mainRoute.endLocation,
          lon: mainRoute.endLocation.lng,
        },
      };

      const validationResponse = await validateRoute(
        resolvedPickupWithLon, 
        resolvedDropWithLon, 
        resolvedMainRoute
      );
      ...
      if (validationResponse.isValid) {
        ...
        // Calculate cost and add passenger
        await calculateOptimizedCost(resolvedPickup, resolvedDrop);
      ...
      setValidationMessage({
        type: 'error',
        message: 'An error occurred while validating the route. Please try again.'
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          {loading ? 'Validating...' : 'Add Passenger'}
        </button>
      </form>
    </div>
  );
