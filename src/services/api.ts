...
export const api = {
  ...
  ,
  async validateRoute(pickup: { lat: number, lon: number }, drop: { lat: number, lon: number }, ownerRoute: { startLocation: { lat: number, lng: number, address: string }, endLocation: { lat: number, lng: number, address: string }, distance: number }) {
    try {
      // Log data to ensure it's correctly formatted
      console.log('Validating route with data:', { pickup, drop, ownerRoute });
      
      const response = await axiosInstance.post(`${API_URL}/validate-route`, {
        pickup,
        drop,
        ownerRoute: {
          startLocation: ownerRoute.startLocation,
          endLocation: ownerRoute.endLocation,
          distance: ownerRoute.distance,
        },
      });

      // Check if the response indicates the route is valid
      if (response.data.isValid) {
        return {
          isValid: true,
          message: 'Route is valid and matches!',
        };
      } else {
        return {
          isValid: false,
          message: response.data.message || 'Route validation failed.',
        };
      }
    } catch (error) {
      console.error('Error in validateRoute:', error);
      handleApiError(error);
      return {
        isValid: false,
        message: 'An error occurred while validating the route.',
      };
    }
  },
